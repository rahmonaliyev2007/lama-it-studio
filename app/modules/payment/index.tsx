  ...
  return (
    <section className="containers" id="payments">
      ...
      <div className="w-full h-screen flex items-center justify-start bg-black text-white relative overflow-hidden">
        ...
        {/* Rotating elements */}
        <div className="absolute min-w-[600px] h-[600px] border border-dashed border-[#898989] rounded-[50%] flex items-center justify-center translate-x-[15%]" style={{ transform: `rotate(${rotation}deg)`, transition: "transform 0.77s ease-in-out", }}>
          {items.map((item, index) => {
            const isActive = index === activeIndex;
            const shouldShowDescription = index === showDescriptionIndex;

            const rotateBase = (360 / items.length) * index;
            const commonTransform = `rotate(${rotateBase}deg) translate(0, -300px) rotate(-${rotation + rotateBase}deg)`;

            if (item.title === "plus") {
              return (
                <div key={index} className="absolute flex items-center justify-center rounded-[50%] border border-dashed border-[#898989] bg-black w-[80px] h-[80px] z-0 transition-all duration-1000 ease-in-out overflow-hidden" style={{ transform: commonTransform }} >
                  <div className="w-[80px] h-[80px] flex items-center justify-center"> {item.icon} </div>
                </div>
              );
            }
            ...
            return (
              <div key={index} className={`absolute flex items-center justify-start bg-white text-black rounded-[30px] p-2 transition-all duration-[0.95s] ease-in-out overflow-hidden ${isActive && shouldShowDescription ? "w-[600px] h-auto z-10" : "w-[80px] h-[80px] z-0"}`}
                ...
                <div className={`${isActive && shouldShowDescription ? "opacity-100 visible max-h-96" : "opacity-0 invisible max-h-0"} min-w-[400px] duration-500`}>
                  <h4 className="font-bold text-2xl text-black">{item.title}</h4>
                  <p className="text-[16px] leading-[90%] text-black font-bold transition-all duration-1000 ease-in-out transform w-[400px]">{item.description}</p>
                </div>
              </div>
            );
          })}
